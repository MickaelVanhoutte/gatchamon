...
export interface MoveSpriteOptions {
	...
	returnEasing?: EasingType | string;
}
...
export class AnimationEngine {
	...
	getEffectZIndex(isPlayer: boolean): number {
		const ABOVE_OPPONENT_SPRITES = 200;
		const BELOW_ALLY_SPRITES = 4;
		return isPlayer ? ABOVE_OPPONENT_SPRITES : BELOW_ALLY_SPRITES;
	}
		...
		timeline
			...
			.add(() => {
				this.showSpriteEffect(effectName, defender, {
					...
					scale: 1.0,
					tint: typeColor,
					zIndex: this.getEffectZIndex(attacker.isPlayer)
				});
