	...
	if (typeEffect && typeEffect !== 'impact') {
		effects.push(
			engine.showSpriteEffect(typeEffect, target, {
				...
				scale: 1.0,
				tint: typeColor,
				zIndex: engine.getEffectZIndex(attacker.isPlayer) - 1
			})
		);
	}
...
const PUNCH_MOVES = [
	...
	'mach-punch', 'focus-punch', 'dynamic-punch', 'drain-punch',
	'power-up-punch', 'sky-uppercut', 'bullet-punch', 'shadow-punch',
	'meteor-mash', 'hammer-arm', 'sucker-punch', 'plasma-fists'
];

const KICK_MOVES = [
	'mega-kick', 'jump-kick', 'rolling-kick', 'hi-jump-kick',
	'low-kick', 'blaze-kick', 'stomp', 'double-kick',
	'low-sweep', 'high-horsepower', 'pyro-ball'
];

const SLASH_MOVES = [
	'cut', 'slash', 'fury-cutter', 'night-slash', 'x-scissor',
	'psycho-cut', 'leaf-blade', 'air-slash', 'razor-shell',
	'sacred-sword', 'secret-sword', 'smart-strike', 'solar-blade',
	'behemoth-blade', 'wicked-blow'
];

const CLAW_MOVES = [
	'scratch', 'shadow-claw', 'dragon-claw', 'metal-claw',
	'crush-claw', 'fury-swipes', 'aerial-ace'
];

const BITE_MOVES = [
	'bite', 'crunch', 'fire-fang', 'ice-fang', 'thunder-fang',
	'poison-fang', 'hyper-fang', 'super-fang', 'psychic-fangs',
	'fishious-rend', 'jaw-lock', 'clamp'
];

const TACKLE_MOVES = [
	'tackle', 'body-slam', 'take-down', 'double-edge', 'headbutt',
	'slam', 'quick-attack', 'extreme-speed', 'rollout', 'rapid-spin',
	'facade', 'return', 'frustration', 'horn-attack', 'peck',
	'drill-peck', 'wing-attack', 'brave-bird', 'head-smash',
	'iron-head', 'zen-headbutt', 'skull-bash', 'wild-charge',
	'volt-tackle', 'flare-blitz', 'giga-impact', 'last-resort',
	'retaliate', 'head-charge', 'raging-bull', 'collision-course',
	'lunge', 'steamroller', 'first-impression', 'pursuit', 'payback',
	'feint-attack', 'thief', 'punishment', 'reversal', 'seismic-toss',
	'wake-up-slap', 'flying-press', 'submission', 'circle-throw',
	'vital-throw', 'storm-throw', 'astonish', 'lick', 'shadow-bone',
	'ice-ball', 'avalanche', 'flail', 'endeavor', 'rock-climb',
	'feint', 'chip-away', 'thrash', 'fake-out', 'smelling-salts',
	'secret-power', 'heart-stamp'
];
...
PUNCH_MOVES.forEach((move) => { physicalMoves[move] = punchAnimation; });
KICK_MOVES.forEach((move) => { physicalMoves[move] = kickAnimation; });
SLASH_MOVES.forEach((move) => { physicalMoves[move] = slashAnimation; });
CLAW_MOVES.forEach((move) => { physicalMoves[move] = clawAnimation; });
BITE_MOVES.forEach((move) => { physicalMoves[move] = biteAnimation; });
TACKLE_MOVES.forEach((move) => { physicalMoves[move] = tackleAnimation; });
CHOP_MOVES.forEach((move) => { physicalMoves[move] = chopAnimation; });
WATER_CONTACT_MOVES.forEach((move) => { physicalMoves[move] = waterContactAnimation; });
WATER_PUNCH_MOVES.forEach((move) => { physicalMoves[move] = waterPunchAnimation; });
WATER_TAIL_MOVES.forEach((move) => { physicalMoves[move] = waterTailAnimation; });
WATER_DASH_MOVES.forEach((move) => { physicalMoves[move] = waterDashAnimation; });
WATER_BURST_MOVES.forEach((move) => { physicalMoves[move] = waterBurstAnimation; });
QUAKE_MOVES.forEach((move) => { physicalMoves[move] = quakeAnimation; });
AERIAL_MOVES.forEach((move) => { physicalMoves[move] = aerialAnimation; });
