...
async function fieldAnimation(engine: AnimationEngine, context: MoveContext): Promise<void> {
	...
	await engine.showSpriteEffect('rock', targetSprite, {
		...
		scale: 1.5,
		tint: color,
		zIndex: engine.getEffectZIndex(targetSprite.isPlayer)
	});
...
const MULTI_HIT_MOVES = [
	...
	'spike-cannon', 'barrage', 'bone-rush', 'arm-thrust',
	'bullet-seed', 'icicle-spear', 'rock-blast', 'water-shuriken',
	'tail-slap', 'triple-kick', 'triple-axel', 'scale-shot',
	'dual-wingbeat', 'surging-strikes', 'twineedle', 'gear-grind'
];
...
MULTI_HIT_MOVES.forEach((move) => { otherMoves[move] = multiHitAnimation; });
OHKO_MOVES.forEach((move) => { otherMoves[move] = ohkoAnimation; });
FIELD_MOVES.forEach((move) => { otherMoves[move] = fieldAnimation; });
TRANSFORM_MOVES.forEach((move) => { otherMoves[move] = transformAnimation; });
WEATHER_MOVES.forEach((move) => { otherMoves[move] = weatherAnimation; });
TERRAIN_MOVES.forEach((move) => { otherMoves[move] = terrainAnimation; });
SIZE_CHANGE_MOVES.forEach((move) => { otherMoves[move] = sizeChangeAnimation; });
