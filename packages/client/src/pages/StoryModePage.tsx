...
import { isDifficultyUnlocked, getRegionStatus, getRegionStars } from './storyMode/storyModeHelpers';
import { getArcWaypoints } from './storyMode/regionWaypoints';
...
export function StoryModePage() {
  ...
  useEffect(() => {
    if (selectedRegionId) {
      serverApi.getFirstClears().then(firstClears => {
        ...
        setFloors(floorList);
      }).catch(err => reportError('StoryModePage.firstClears', err));
    }
  ...
  function handleDifficultyChange(diff: Difficulty) {
    if (!isDifficultyUnlocked(diff, currentArc, progress)) return;
    setDifficulty(diff);
  ...
  const pathWaypoints = getArcWaypoints(selectedArc);
    ...
    <div className="page story-page">
      ...
      <div className="world-map" ref={worldMapRef} data-horizontal-scroll>
        <div className="map-scroll">
          ...
          const status = getRegionStatus(region.id, regionProgress);
          const stars = getRegionStars(region.id, regionProgress);
      ...
      <div className="map-header-left">
        <div className="difficulty-tabs">
          {DIFFICULTIES.map(d => {
            const unlocked = isDifficultyUnlocked(d.key, currentArc, progress);
            return (
              <button
                key={d.key}
